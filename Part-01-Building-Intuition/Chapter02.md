...
> **一句话总结**：不管模型有多大、训练花了多少钱，最终大模型就是两个文件——一个存参数，一个跑推理也就是一个负责编码输入存储知识，一个负责解码推理输出知识。理解这一点，你就抓住了 LLM 的本质。
---
...
GPT以及各种大模型都不是一次性把整个答案想好再输出，而是**一个字一个字地往外"蹦"进行输出。每生成一个字，它都要重新思考：基于目前已经生成的所有内容，下一个字应该是什么？**
...
一个问题可能有无数种合理的回答。如果让模型一次性输出完整答案，它需要同时**考虑所有可能的组合**——这个计算量是天文数字。

但如果我们把**问题简化成"给定前面的内容，下一个字是什么"**，问题就变得可控了。每一步只需要从几万个候选字里选一个最合适的。
...
GPT或者说所有大语言模型的 本质上就是在做这件事：**基于大量文本数据，学习"在什么上下文之后，什么字出现的概率最高"**。
...
这意味着**如果你有参数文件，你可以用 500 行代码让它"说话"。**
...
| **概念**         | **解释**                                   |
| ---------- | ------------------------------------ |
| **自回归生成**      | **GPT一个字一个字地生成答案，每次基于之前的内容预测下一个字（马尔可夫）** |
...
> **大模型不是魔法，它就是两个文件：一个存参数存知识，一个跑推理会说话。理解这一点，是理解后续所有内容的基础。**
